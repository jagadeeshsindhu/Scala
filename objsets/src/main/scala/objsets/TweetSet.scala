 ...
 * subtree are smaller than the tweet at `b`. The elemnts in the right subtree are
 * larger.
 *
...
*/
  def mostRetweeted: Tweet = {
    ...
    def mostTwoTweets(tw1: Tweet, tw2: Tweet): Tweet = {
      if (tw1.retweets > tw2.retweets)
        tw1
      else
        tw2
    }
    ...
    if (left.isEmpty && right.isEmpty)
      ...
      mostTwoTweets(right.mostRetweeted, elem)
    else if (right.isEmpty)
      mostTwoTweets(left.mostRetweeted, elem)
    else (mostTwoTweets(left.mostRetweeted, mostTwoTweets(right.mostRetweeted, elem)))
  }
...
object GoogleVsApple {
 ...
 // val allAndroid = allTweets.filter()
