...
object FunSets {
  ...
  def map(s: Set, f: Int => Int): Set = {
    def constructMap(unionSet: Set, elem: Int): Set = {    
      if (elem > bound) unionSet
      else if (s(elem)) constructMap(union(unionSet, singletonSet(f(elem))), elem+1)
      else constructMap(unionSet, elem+1)      
    }
    constructMap( x => false, -bound)
  }
